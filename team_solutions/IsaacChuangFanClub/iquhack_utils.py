...
def find_UDG_radius(positions, graph):
    '''Computes the optimal unit disk radius for a particular set of positions and graph.
    
    Args
        position (dict): dictionary containing the coordinates of the nodes in the plane. 
        graph (networkx.Graph): network connectivity graph. This should be a unit disk graph.
    
    Return
        tuple: elements include:
            radius (float): Optimal unit disk radius of the graph
            rmin (float): Minimum distance
            rmax (float): Maximum distance
    '''
    ...
    if rmin>rmax:
        raise BaseException("rmin > rmax, Graph is not a unit disk graph!")
